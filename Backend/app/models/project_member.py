...
class ProjectMember(db.Model):
    ...
    project = db.relationship(
        "Project", foreign_keys=[project_id], back_populates="project_members"
    )
    user = db.relationship("User", foreign_keys=[user_id], back_populates="project_memberships")
    added_by = db.relationship(
        "User", foreign_keys=[added_by_user_id], back_populates="memberships_added"
    )
    removed_by = db.relationship(
        "User", foreign_keys=[removed_by_user_id], back_populates="memberships_removed"
    )
