...
def test_get_time_entries_with_filters_returns_200(client, time_entry_factory, user_factory, project_factory):
	user = user_factory(email="route-time-entry-filter-user@test.com")
	project = project_factory()
	time_entry_factory(user=user, project=project, work_date=date.today() - timedelta(days=2), description="Older")
	time_entry_factory(user=user, project=project, work_date=date.today() - timedelta(days=1), description="Newer")
...
def test_update_time_entry_returns_403_when_different_user(client, time_entry_factory, user_factory):
...
def test_delete_time_entry_by_id_returns_200(client, time_entry_factory):
	...
	response = client.delete(
		f"/time-entries/{time_entry.id}",
		json={"user_id": str(time_entry.user_id)}
	)
...
def test_delete_time_entry_by_id_returns_403_when_different_user(client, time_entry_factory, user_factory):
	time_entry = time_entry_factory(description="Delete from route")
	different_user = user_factory(email="route-time-entry-delete-different-user@test.com")

	response = client.delete(
		f"/time-entries/{time_entry.id}",
		json={"user_id": str(different_user.id)}
	)
