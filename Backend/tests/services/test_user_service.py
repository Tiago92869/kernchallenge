...
def test_update_user_sucess(user_factory):
    ...
    user = UserService.update_user(
        user_id=user_db.id, email="tiago@gmail.com", firstname="Pedro", lastname="Jonas"
    )
...
def test_update_user_wrong_email_format(user_factory):
    ...
    with pytest.raises(ValidationError) as exc_info:
        UserService.update_user(
            user_id=user_db.id, email="invalidemail", firstname="Pedro", lastname="Jonas"
        )
...
def test_update_password_sucess(user_factory):
    ...
    UserService.update_password(
        user_id=user_db.id, old_password="password123", new_password="newpassword123"
    )
...
def test_update_password_empty_password(user_factory):
    ...
    with pytest.raises(ValidationError) as exc_info:
        UserService.update_password(user_id=user_db.id, old_password="password123", new_password="")
...
def test_update_password_current_password_same_as_new_password(user_factory):
    ...
    with pytest.raises(ValidationError) as exc_info:
        UserService.update_password(
            user_id=user_db.id, old_password="password123", new_password="password123"
        )
...
def test_update_password_incorrect_old_password(user_factory):
    ...
    with pytest.raises(ValidationError) as exc_info:
        UserService.update_password(
            user_id=user_db.id, old_password="wrongpassword", new_password="newpassword123"
        )
...
def test_does_user_exist_and_active_returns_false_for_missing_user(app):
    with app.app_context():
        user_exists = UserService.does_user_exist_and_active(
            UUID("5540e840-e29b-41d4-a716-446655440000")
        )
